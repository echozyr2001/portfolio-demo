...
import {
	POST as importPOST,
	GET as importGET,
} from "@/app/api/admin/content/import/route";
import {
	POST as exportPOST,
	GET as exportGET,
} from "@/app/api/admin/content/export/route";
	...
	describe("Import API", () => {
		describe("POST /api/admin/content/import", () => {
			it("should successfully import files", async () => {
				const { contentImportExportService } = await import(
					"@/lib/content-import-export"
				);
				...
				// Create mock files
				const file1 = new File(["# Test Post 1"], "post1.mdx", {
					type: "text/markdown",
				});
				const file2 = new File(["# Test Post 2"], "post2.mdx", {
					type: "text/markdown",
				});
				...
				const request = new NextRequest(
					"http://localhost:3000/api/admin/content/import",
					{
						method: "POST",
						body: formData,
					},
			...
			it("should return error for missing config", async () => {
				const formData = new FormData();
				const file = new File(["# Test"], "test.mdx", {
					type: "text/markdown",
				});
		...
		describe("GET /api/admin/content/import", () => {
			it("should return import configuration options", async () => {
				...
				expect(data.data.conflictResolutionOptions).toEqual([
					"skip",
					"overwrite",
					"rename",
				]);
	...
	describe("Export API", () => {
		...
		describe("GET /api/admin/content/export", () => {
			it("should return export configuration options", async () => {
				...
				expect(data.data.contentTypes).toEqual(["posts", "projects"]);
				expect(data.data.statusOptions).toEqual([
					"draft",
					"published",
					"archived",
				]);
