...
describe('Media Utils', () => {
  ...
  describe('needsOptimization', () => {
    ...
    it('should use custom limits', () => {
      expect(needsOptimization(1000, 800, 1024 * 1024, 800, 600, 512 * 1024)).toBe(true);
      expect(needsOptimization(600, 400, 256 * 1024, 800, 600, 512 * 1024)).toBe(false);
    });
  });

  describe('generateResponsiveSizes', () => {
    it('should generate appropriate responsive sizes', () => {
      expect(generateResponsiveSizes(2000)).toEqual([320, 640, 768, 1024, 1280, 1920]);
  ...
  describe('createMediaUrl', () => {
    ...
    it('should create thumbnail URLs', () => {
      expect(createMediaUrl('test-id', { thumbnail: true })).toBe('/api/media/test-id?thumbnail=true');
    });

    it('should create info URLs', () => {
      expect(createMediaUrl('test-id', { info: true })).toBe('/api/media/test-id?info=true');
    });

    it('should combine multiple options', () => {
      expect(createMediaUrl('test-id', { thumbnail: true, info: true }))
        .toBe('/api/media/test-id?thumbnail=true&info=true');
    });
  });
  ...
  describe('generateAltTextSuggestion', () => {
    it('should generate alt text from filename', () => {
      ...
      expect(generateAltTextSuggestion('beach_sunset.png')).toBe('Beach Sunset');
      expect(generateAltTextSuggestion('profilePicture.jpeg')).toBe('Profile Picture');
    ...
    it('should handle complex filenames', () => {
      expect(generateAltTextSuggestion('vacation-2024_beach-sunset_final.jpg'))
