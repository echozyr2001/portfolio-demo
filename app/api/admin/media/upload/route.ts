...
export async function POST(request: NextRequest) {
	try {
		...
		if (validationError) {
			return NextResponse.json(validationError.response, {
...
function validateFileBeforeProcessing(
	file: File,
): { response: any; status: number } | null {
	// Check file size
...
export async function GET(request: NextRequest) {
	try {
		...
		const totalSize = filteredFiles.reduce(
			(sum, file) => sum + (file.optimizedSize || file.size),
			0,
		);
		const totalOriginalSize = filteredFiles.reduce(
		...
		return NextResponse.json({
			...
			stats: {
				...
				totalOriginalSizeMB:
					Math.round((totalOriginalSize / (1024 * 1024)) * 100) / 100,
				averageCompressionRatio:
					filteredFiles.length > 0
						? filteredFiles.reduce(
								(sum, file) => sum + (file.compressionRatio || 1),
								0,
