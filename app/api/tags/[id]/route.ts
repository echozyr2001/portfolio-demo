...
import { postTags, tags } from "@/lib/schema";
import {
	createErrorResponse,
...
import { idParamSchema, updateTagSchema } from "@/lib/validations";
...
export async function GET(
	...
	try {
		...
		return createErrorResponse("INTERNAL_ERROR", "Failed to fetch tag", 500);
	}
}
...
export async function PUT(
	...
	try {
		...
		if (Object.keys(updateData).length === 0) {
			...
			return createSuccessResponse(currentTagWithCount[0], "No changes made");
		}
		...
		return createSuccessResponse(updatedTag[0], "Tag updated successfully");
	} catch (error) {
		if (error instanceof ZodError) {
			return handleValidationError(error);
		}
		...
		return createErrorResponse("INTERNAL_ERROR", "Failed to update tag", 500);
	}
}
...
export async function DELETE(
	...
	try {
		...
		return createErrorResponse("INTERNAL_ERROR", "Failed to delete tag", 500);
	}
}
