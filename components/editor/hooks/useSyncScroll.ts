...
export function useSyncScroll(options: SyncScrollOptions = {}) {
  ...
  // 设置编辑器引用
  const setEditorRef = useCallback((editorInstance: editor.IStandaloneCodeEditor | null) => {
  ...
  // 从编辑器滚动到预览
  const syncEditorToPreview = useCallback((scrollTop: number, scrollHeight: number, clientHeight: number) => {
    if (!enabled || !previewRef.current || isScrollingFromPreview.current) return;
  ...
  // 从预览滚动到编辑器
  const syncPreviewToEditor = useCallback((scrollTop: number, scrollHeight: number, clientHeight: number) => {
    if (!enabled || !editorRef.current || isScrollingFromEditor.current) return;
    ...
    const editorMaxScrollTop = Math.max(0, editorScrollHeight - editorClientHeight);
  ...
  useEffect(() => {
    ...
    previewElement.addEventListener('scroll', debouncedHandlePreviewScroll, { passive: true });

    return () => {
      previewElement.removeEventListener('scroll', debouncedHandlePreviewScroll);
      if (timeoutId) clearTimeout(timeoutId);
    };
