...
import type { BlogFormData, ProjectFormData, MDXValidationResult } from './types/mdx'
 ...
 */
export function validateBlogData(data: Partial<BlogFormData>): MDXValidationResult {
  const errors: Array<{ type: 'syntax' | 'component' | 'runtime', message: string }> = []
  ...
  // Excerpt validation
  if (data.excerpt && data.excerpt.length > VALIDATION_RULES.blog.excerpt.maxLength) {
    warnings.push(`Excerpt should be less than ${VALIDATION_RULES.blog.excerpt.maxLength} characters`)
  }
  
  // Tags validation
  if (data.tags && data.tags.length > VALIDATION_RULES.blog.tags.maxItems) {
    warnings.push(`Consider using fewer than ${VALIDATION_RULES.blog.tags.maxItems} tags`)
  }
 ...
 */
export function validateProjectData(data: Partial<ProjectFormData>): MDXValidationResult {
  const errors: Array<{ type: 'syntax' | 'component' | 'runtime', message: string }> = []
  const warnings: string[] = []
  ...
  } else if (data.description.length > VALIDATION_RULES.project.description.maxLength) {
    errors.push({ 
      type: 'syntax', 
  ...
  } else if (data.shortDescription.length > VALIDATION_RULES.project.shortDescription.maxLength) {
    errors.push({ 
      type: 'syntax', 
  ...
  if (!data.technologies || data.technologies.length === 0) {
    errors.push({ type: 'syntax', message: 'At least one technology is required' })
  } else if (data.technologies.length > VALIDATION_RULES.project.technologies.maxItems) {
    warnings.push(`Consider using fewer than ${VALIDATION_RULES.project.technologies.maxItems} technologies`)
  }
