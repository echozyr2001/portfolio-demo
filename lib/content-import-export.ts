import { z } from "zod";
import { db } from "./db";
import { posts, projects, categories, tags, postTags, projectTags } from "./schema";
...
export class ContentImportExportService {
  ...
  async importMDXFiles(
    ...
    for (const file of files) {
      try {
        ...
        // 解析MDX内容
        const parseResult = await this.parseMDXFile(validatedFile.content, options.validateContent);
        ...
        const existingContent = await this.checkContentExists(
          parseResult.frontmatter.slug || this.generateSlugFromFilename(file.filename),
          contentType
        );
        ...
        if (existingContent) {
          switch (options.conflictResolution) {
            case "skip":
              result.skipped++;
              result.warnings.push(`Skipped ${file.filename}: content already exists`);
              continue;
            case "overwrite":
              await this.updateExistingContent(existingContent.id, parseResult, contentType, options);
              result.imported++;
              break;
            case "rename":
              ...
              result.imported++;
              result.warnings.push(`Renamed ${file.filename} to avoid conflict`);
              break;
          }
        } else {
          await this.createNewContent(parseResult, contentType, options);
    ...
    try {
      ...
      if (options.contentTypes.includes("posts")) {
        ...
        for (const post of postsData) {
          const mdxContent = await this.generateMDXContent(post, "post", options.includeMetadata);
      ...
      if (options.contentTypes.includes("projects")) {
        ...
        for (const project of projectsData) {
          const mdxContent = await this.generateMDXContent(project, "project", options.includeMetadata);
  ...
  private async createNewContent(
    ...
    const id = nanoid();
    const now = new Date();
    const slug = parseResult.frontmatter.slug || this.generateSlugFromTitle(parseResult.frontmatter.title);

    // 处理分类和标签
    const categoryId = await this.handleCategory(parseResult.frontmatter.category, options.createMissingCategories);
    const tagIds = await this.handleTags(parseResult.frontmatter.tags || [], options.createMissingTags);
    ...
    if (contentType === "posts") {
      ...
      await db.insert(posts).values({
        ...
        publishedAt: parseResult.frontmatter.publishedAt ? new Date(parseResult.frontmatter.publishedAt) : null,
        categoryId,
        createdAt: now,
        updatedAt: now,
      });
   ...
   * 处理分类
   */
  private async handleCategory(categoryName: string | undefined, createMissing: boolean): Promise<string | null> {
   ...
   * 处理标签
   */
  private async handleTags(tagNames: string[], createMissing: boolean): Promise<string[]> {
   ...
   * 获取要导出的文章数据
   */
  private async getPostsForExport(status?: ("draft" | "published" | "archived")[]) {
    let query = db.select({
      id: posts.id,
      title: posts.title,
      slug: posts.slug,
   ...
   * 获取要导出的项目数据
   */
  private async getProjectsForExport(status?: ("draft" | "published" | "archived")[]) {
    let query = db.select({
      id: projects.id,
   ...
   * 生成唯一slug
   */
  private async generateUniqueSlug(baseSlug: string, contentType: "posts" | "projects"): Promise<string> {
    let counter = 1;
    let slug = baseSlug;
