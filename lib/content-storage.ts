...
export interface ContentStorageService {
   ...
   */
  saveContent(id: string, mdxContent: string, type: 'post' | 'project'): Promise<void>;
...
export class ContentNotFoundError extends ContentStorageError {
  constructor(id: string, type: string) {
    super(`Content not found: ${type}/${id}`, 'CONTENT_NOT_FOUND', { id, type });
  }
}
...
export class DatabaseContentStorage implements ContentStorageService {
  async saveContent(id: string, mdxContent: string, type: 'post' | 'project'): Promise<void> {
    try {
...
export class ContentStorageFactory {
   ...
   */
  static create(storageType: 'database' | 's3' = 'database'): ContentStorageService {
    switch (storageType) {
      case 's3':
   ...
   */
  static createForRecord(record: { contentStorageType: string | null }): ContentStorageService {
   ...
   */
  static async createForContent(id: string, type: 'post' | 'project'): Promise<ContentStorageService> {
    try {
      const table = type === 'post' ? posts : projects;
      const record = await db.select({ 
        contentStorageType: table.contentStorageType 
   ...
   */
  static validateStorageConfig(storageType: 'database' | 's3'): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];

    if (storageType === 's3') {
      if (!this.isS3Configured()) {
        errors.push('S3 storage is not properly configured. Missing environment variables.');
      }
...
export class ContentStorageUtils {
  ...
  static async batchMigrate(
    ...
    toStorage: 'database' | 's3',
    batchSize: number = 10
  ): Promise<{ success: number; failed: Array<{ id: string; type: string; error: string }> }> {
    const results = {
      success: 0,
      ...
      await Promise.allSettled(
        batch.map(async (item) => {
          try {
            ...
            const currentStorage = await ContentStorageFactory.createForContent(item.id, item.type);
            const table = item.type === 'post' ? posts : projects;
            const record = await db.select({ contentStorageType: table.contentStorageType })
              .from(table)
              .where(eq(table.id, item.id))
              .limit(1);

            const fromStorage = record[0]?.contentStorageType as 'database' | 's3' || 'database';
            
            await this.migrateContent(item.id, item.type, fromStorage, toStorage);
            results.success++;
          } catch (error) {
            results.failed.push({
              id: item.id,
  ...
  static async getStorageStats(): Promise<{
    ...
    const [databasePosts, s3Posts, databaseProjects, s3Projects, totalPosts, totalProjects] = await Promise.all([
      db.select({ count: posts.id }).from(posts).where(eq(posts.contentStorageType, 'database')),
      db.select({ count: posts.id }).from(posts).where(eq(posts.contentStorageType, 's3')),
      db.select({ count: projects.id }).from(projects).where(eq(projects.contentStorageType, 'database')),
      db.select({ count: projects.id }).from(projects).where(eq(projects.contentStorageType, 's3')),
