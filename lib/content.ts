...
export const getAllContent = cache(
  <T extends Frontmatter>(type: string): Content<T>[] => {
...
export const getContentBySlug = <T extends Frontmatter>(
  type: string,
  slug: string
): Content<T> | undefined => {
...
export const getUniqueFrontmatterValues = <T extends Frontmatter>(
  type: string,
  key: keyof T
): string[] => {
  const allContent = getAllContent<T>(type);
  ...
  allContent.forEach((item) => {
    ...
    if (typeof value === "string" && value) {
      ...
      value.forEach((v: any) => typeof v === "string" && v && values.add(v));
    }
  });
