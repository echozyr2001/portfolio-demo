...
export function getComponentSuggestions(componentName: string): string[] {
  ...
  if (!componentDoc) {
    return [
      `Component '${componentName}' not found. Available components: ${Object.keys(componentDocs).join(", ")}`,
    ];
  }
...
export function formatValidationErrors(result: ValidationResult): string {
  ...
  if (result.errors.length > 0) {
    ...
    result.errors.forEach((error) => {
      errorMessages.push(
        `  - ${error.component}${error.prop ? `.${error.prop}` : ""}: ${error.message}`
      );
    });
  }
  ...
  if (result.warnings.length > 0) {
    ...
    result.warnings.forEach((warning) => {
      errorMessages.push(
        `  - ${warning.component}${warning.prop ? `.${warning.prop}` : ""}: ${warning.message}`
      );
    });
  }
