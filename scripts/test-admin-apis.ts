...
`,
		...
		if (retrievedPost.length > 0) {
			...
			console.log(
				`   - MDX Content Length: ${retrievedPost[0].mdxContent?.length || 0} characters`,
			);
		}
		...
		if (retrievedProject.length > 0) {
			...
			console.log(
				`   - Technologies: ${JSON.stringify(retrievedProject[0].technologies)}`,
			);
			console.log(
				`   - MDX Content Length: ${retrievedProject[0].mdxContent?.length || 0} characters`,
			);
		}
		...
		const projectStorage = ContentStorageFactory.createForRecord(
			retrievedProject[0],
		);

		const postContent = await postStorage.getContent(postId, "post");
		const projectContent = await projectStorage.getContent(
			projectId,
			"project",
		);
		...
		console.log(
			`   - Post content retrieved: ${postContent.length} characters`,
		);
		console.log(
			`   - Project content retrieved: ${projectContent.length} characters`,
		);
