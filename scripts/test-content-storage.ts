...
async function runTests() {
  ...
  try {
    ...
    console.log('   - Database storage created:', databaseStorage.constructor.name);
    ...
    console.log('   - Empty content validation:', emptyResult.isValid, '(should be false)');
    ...
    console.log('\n✅ Test 4: Error classes');
    const contentError = new ContentStorageError('Test error', 'TEST_CODE', { test: true });
    console.log('   - ContentStorageError created:', contentError.name, contentError.code);
    ...
    console.log('   - ContentValidationError created:', validationError.message);
    ...
    try {
      ...
      console.log('   - Storage stats error (expected if no data):', error.message);
    }
    ...
    console.log('   ✅ S3ContentStorage interface created (not yet implemented)');
