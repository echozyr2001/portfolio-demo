...
`;
  ...
  try {
    ...
    const parseResult = await (service as any).parseMDXFile(testMDXContent, false);
    ...
    console.log("   - Reading time:", parseResult.metadata.readingTime, "minutes");
    console.log("   - Word count:", parseResult.metadata.wordCount);
    console.log("   - Excerpt:", parseResult.metadata.excerpt.substring(0, 50) + "...");
  } catch (error) {
  ...
  try {
    ...
    console.log("   Valid frontmatter:", JSON.stringify(validFrontmatter, null, 2));
    console.log("   Invalid frontmatter:", JSON.stringify(invalidFrontmatter, null, 2));
