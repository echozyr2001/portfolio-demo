...
class MediaAPITester {
  ...
  private async testFileUpload(): Promise<TestResult> {
    ...
    try {
      ...
      const file = new File([fileBuffer], 'test-image.svg', { type: 'image/svg+xml' });
  ...
  private async testGetMediaInfo(mediaId: string): Promise<TestResult> {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/media/${mediaId}?info=true`);
      const data = await response.json();
  ...
  private async testGetMediaFile(mediaId: string): Promise<TestResult> {
    ...
    try {
      ...
      this.results.push(result);
      console.log(`✅ ${testName} - Got ${buffer.byteLength} bytes of ${contentType}`);
      return result;
