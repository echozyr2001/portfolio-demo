...
class MediaValidationTester {
	...
	private async testEmptyFile(): Promise<ValidationTestResult> {
		...
		try {
			...
			if (response.status === 400 && data.code === "EMPTY_FILE") {
				...
				throw new Error(
					`Expected 400 with EMPTY_FILE, got ${response.status} with ${data.code}`,
				);
			}
		} catch (error) {
			const result: ValidationTestResult = {
	...
	private async testInvalidFileType(): Promise<ValidationTestResult> {
		...
		try {
			...
			const invalidFile = new File(["fake pdf content"], "document.pdf", {
			...
			if (response.status === 415 && data.code === "INVALID_FILE_TYPE") {
				...
				throw new Error(
					`Expected 415 with INVALID_FILE_TYPE, got ${response.status} with ${data.code}`,
				);
			}
		} catch (error) {
			const result: ValidationTestResult = {
	...
	private async testFileTooLarge(): Promise<ValidationTestResult> {
		...
		try {
			...
			const largeFile = new File([largeBuffer], "large-image.jpg", {
				type: "image/jpeg",
			});
			...
			if (response.status === 413 && data.code === "FILE_TOO_LARGE") {
				...
				throw new Error(
					`Expected 413 with FILE_TOO_LARGE, got ${response.status} with ${data.code}`,
				);
			}
		} catch (error) {
			const result: ValidationTestResult = {
	...
	private async testMissingFile(): Promise<ValidationTestResult> {
		...
		try {
			...
			if (response.status === 400 && data.code === "NO_FILE") {
				...
				throw new Error(
					`Expected 400 with NO_FILE, got ${response.status} with ${data.code}`,
				);
			}
		} catch (error) {
			const result: ValidationTestResult = {
	...
	private async testInvalidFilename(): Promise<ValidationTestResult> {
		...
		try {
			...
			const fileWithEmptyName = new File(["test content"], "", {
				type: "image/jpeg",
			});
			...
			if (response.status === 400 && data.code === "INVALID_FILENAME") {
				...
				throw new Error(
					`Expected 400 with INVALID_FILENAME, got ${response.status} with ${data.code}`,
				);
			}
		} catch (error) {
			const result: ValidationTestResult = {
	...
	private async testValidFileTypes(): Promise<ValidationTestResult> {
		...
		try {
			for (const fileType of validTypes) {
				...
				const file = new File([fileContent], `test.${fileType.ext}`, {
					type: fileType.type,
				});
			...
			for (const id of uploadedIds) {
				try {
					await fetch(`${BASE_URL}/api/admin/media/${id}`, {
						method: "DELETE",
					});
				} catch (error) {
			...
			const result: ValidationTestResult = {
				...
				data: {
					acceptedTypes: uploadedIds.length,
			...
			if (result.success) {
				...
				console.log(
					`❌ ${testName} - Only ${uploadedIds.length}/${validTypes.length} types accepted`,
				);
			}
	...
	private printResults(): void {
		...
		this.results.forEach((result, index) => {
			...
			if (result.expectedError && result.actualError) {
				if (result.success) {
					console.log(
						`   Expected: ${result.expectedError}, Got: ${result.actualError} ✓`,
					);
				} else {
					console.log(
						`   Expected: ${result.expectedError}, Got: ${result.actualError} ✗`,
		...
		if (passed === total) {
			console.log(
				"🎉 All validation tests passed! Error handling is working correctly.",
			);
		} else {
			console.log(
				"⚠️  Some validation tests failed. Please check the error handling.",
			);
		}
	}
}
