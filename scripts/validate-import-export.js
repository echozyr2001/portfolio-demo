...
try {
	...
	const hasServiceClass = serviceContent.includes(
	...
	const hasInterfaces = serviceContent.includes(
		"export interface ImportOptions",
	);

	console.log(
		`   ${hasServiceClass ? "✅" : "❌"} ContentImportExportService class`,
	);
...
try {
	const serviceTest = fs.readFileSync(
		"__tests__/content-import-export.test.ts",
		"utf8",
	);
	...
	const serviceTestHasDescribe = serviceTest.includes(
	...
	console.log(
		`   ${serviceTestHasDescribe ? "✅" : "❌"} Service test structure`,
	);
	console.log(
		`   ${serviceTestHasImportTest ? "✅" : "❌"} Import functionality tests`,
	);
	console.log(
		`   ${serviceTestHasExportTest ? "✅" : "❌"} Export functionality tests`,
	);
	console.log(`   ${apiTestHasRoutes ? "✅" : "❌"} API routes tests`);

	if (
		serviceTestHasDescribe &&
		serviceTestHasImportTest &&
		serviceTestHasExportTest &&
		apiTestHasRoutes
	) {
...
console.log(
	"   - API endpoints: /api/admin/content/import, /api/admin/content/export",
);
console.log("   - Admin UI: /admin/import-export");
console.log(
	"   - Features: Batch import, conflict resolution, ZIP export, validation",
);
