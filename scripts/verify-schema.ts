...
import { media, posts, projects, projectTags } from "../lib/schema";
...
async function verifySchema() {
	...
	try {
		...
		console.log(
			`  ✓ Projects table exists with ${projectsInfo.length} columns`,
		);

		// Test project_tags table
		console.log("✅ Checking project_tags table...");
		const projectTagsInfo = sqlite
			.prepare("PRAGMA table_info(project_tags)")
			.all();
		if (projectTagsInfo.length === 0) {
			throw new Error("Project_tags table does not exist");
		}
		console.log(
			`  ✓ Project_tags table exists with ${projectTagsInfo.length} columns`,
		);
